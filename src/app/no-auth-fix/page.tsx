...
export default function NoAuthFixPage() {
  ...
  const createTestOrder = async () => {
    try {
      setLoading(true);
      addLog('Creating test order via API (no auth required)...');

      // Call our API endpoint that uses service role to bypass RLS
      ...
      addLog(`Customer created with ID: ${data.customer.id}`);
      addLog(`Order created with ID: ${data.order.id}`);
      addLog(`Order history created successfully`);
      addLog('Test order created successfully!');

      if (data.totalOrders) {
        addLog(`Total orders for shop owner: ${data.totalOrders}`);
      }
