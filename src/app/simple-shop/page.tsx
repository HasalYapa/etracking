...
export default function SimpleShopPage() {
  ...
  const fetchOrders = async (userId: string) => {
    try {
      ...
      addLog(`Fetched ${ordersData?.length || 0} orders`);
      ...
      const delivered = ordersData?.filter(order => order.status === 'delivered').length || 0;
      ...
      addLog(`Error in fetchOrders: ${err.message}`);
      setError(err.message);
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      const { data: order, error: orderError } = await supabase
        ...
        .insert({
          ...
          delivery_notes: 'Test order created from simple shop dashboard',
      ...
      addLog(`Error in createTestOrder: ${err.message}`);
      setError(err.message);
      setLoading(false);
    }
  };
  ...
  const handleSignOut = async () => {
    try {
      ...
      addLog(`Error signing out: ${err.message}`);
      setError(err.message);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {loading ? (
                    ...
                    {orders.map((order) => (
                      <tr key={order.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          ...
                          {order.customers?.phone && <div className="text-xs text-gray-400">{order.customers.phone}</div>}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
          ...
          <div className="bg-white rounded-lg shadow-sm p-8 text-center">
            <p className="text-gray-500">Please log in to view your shop dashboard.</p>
            <button
