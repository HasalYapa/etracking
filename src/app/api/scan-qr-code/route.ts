  ...
  try {
    ...
    if (historyError) {
      ...
      try {
        ...
        const sql = `
          INSERT INTO order_history (order_id, status, notes, updated_by, created_at, latitude, longitude)
          VALUES ('${orderId}', '${status}', 'Status updated to ${status}', '${driverId}', '${new Date().toISOString()}', ${latitude || 'NULL'}, ${longitude || 'NULL'})
          RETURNING *;
        `;
